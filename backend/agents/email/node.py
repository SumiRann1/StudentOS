from state import AgentState, get_email_prompt
from config import email_llm_with_tools, router_llm, get_router_prompt, get_current_day, get_current_date, get_current_time
...
def chat_node(state: AgentState):
    ...
    system_prompt = get_email_prompt(current_day, current_date, current_time, user_name)
    if not messages or not isinstance(messages[0], SystemMessage):
        messages = [SystemMessage(content=system_prompt)] + messages

    response = email_llm_with_tools.invoke(messages)
    return {"email_result": [response], "messages": [response]}